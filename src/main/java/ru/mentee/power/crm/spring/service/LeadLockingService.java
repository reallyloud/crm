...
public class LeadLockingService {
  ...
  /**
   * Блокирует два лида в заданном порядке (в одной транзакции). Используется для теста deadlock:
   * один поток вызывает lockTwoLeads(id1, id2), другой — lockTwoLeads(id2, id1).
   */
  @Transactional
  public void lockTwoLeadsInOrder(UUID firstId, UUID secondId) {
    leadRepository
        .findByIdForUpdate(firstId)
        .orElseThrow(() -> new IllegalArgumentException("Lead not found: " + firstId));
    leadRepository
        .findByIdForUpdate(secondId)
        .orElseThrow(() -> new IllegalArgumentException("Lead not found: " + secondId));
  }
