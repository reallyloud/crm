...
class JpaDealControllerTest {
  ...
  @MockitoBean private JpaDealService dealService;
  ...
  void shouldShowConvertForm() throws Exception {
    ...
    mockMvc
        .perform(get("/jpa/deals/convert/{leadId}", leadId))
        .andExpect(status().isOk())
  ...
  void shouldReturn404WhenConvertingNonExistentLead() throws Exception {
    ...
    mockMvc.perform(get("/jpa/deals/convert/{leadId}", leadId)).andExpect(status().isNotFound());
  }
  ...
  void shouldConvertLeadToDealAndRedirect() throws Exception {
    ...
    mockMvc
        .perform(
            post("/jpa/deals/convert").param("leadId", leadId.toString()).param("amount", "50000"))
  ...
  void shouldTransitionDealStatusAndRedirect() throws Exception {
    ...
    mockMvc
        .perform(post("/jpa/deals/{id}/transition", dealId).param("newStatus", "QUALIFIED"))
        .andExpect(status().is3xxRedirection())
