...
class IsolationsTest {
  ...
  private void resetLeadName() {
    Lead lead =
        repository.findById(leadId).orElseThrow(() -> new RuntimeException("Lead not found"));
    lead.setName("Старое имя");
              ...
              status1 -> {
                ...
                log.info(
                    transactionName
                        + isolation.name()
                        + otherTransaction
                        + "имя лида изменено на 'Новое имя!!!'");
                return null;
              });

          entityManager.clear();

          Lead leadAfterUpdate =
              repository.findById(leadId).orElseThrow(() -> new RuntimeException("Lead not found"));
          String secondRead = leadAfterUpdate.getName();

          transactionName = tx1.getName();
          log.info(
              isolation.name() + masterTransaction + "- Второй SELECT: Имя лида = " + secondRead);
