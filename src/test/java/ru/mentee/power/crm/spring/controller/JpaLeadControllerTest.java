...
class JpaLeadControllerTest {
    ...
    void shouldFilterLeadsBySearch() throws Exception {
        ...
        mockMvc.perform(get("/jpa/leads").param("search", "test"))
                .andExpect(status().isOk())
    ...
    void shouldFilterLeadsByStatus() throws Exception {
        ...
        mockMvc.perform(get("/jpa/leads").param("status", "NEW"))
                .andExpect(status().isOk())
    ...
    void shouldShowCreateForm() throws Exception {
        mockMvc.perform(get("/jpa/leads/new"))
                .andExpect(status().isOk())
    ...
    void shouldCreateLeadAndRedirect() throws Exception {
        ...
        mockMvc.perform(post("/jpa/leads")
                        .param("name", "Иван")
    ...
    void shouldShowEditForm() throws Exception {
        ...
        mockMvc.perform(get("/jpa/leads/{id}/edit", id))
                .andExpect(status().isOk())
                .andExpect(model().attributeExists("lead"))
    ...
    void shouldUpdateLeadAndRedirect() throws Exception {
        ...
        Lead lead = createTestLead();
        lead.setId(id);
        when(leadService.update(eq(id), any(), any(), any(), any(), any()))
                .thenReturn(lead);

        mockMvc.perform(post("/jpa/leads/{id}", id)
        ...
        verify(leadService).update(eq(id), eq("Обновленный"), eq("updated@test.com"),
                eq("89999999"), eq("Google"), eq(LeadStatus.CONTACTED));
    }
    ...
    void shouldDeleteLeadAndRedirect() throws Exception {
        ...
        mockMvc.perform(post("/jpa/leads/{id}/delete", id))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/jpa/leads"));
