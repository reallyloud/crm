...
class LeadControllerTest {
  ...
  void shouldFindOnlyOneLead() throws Exception {
    ...
    mockMvc
        .perform(get("/leads").param("search", "ivan"))
  ...
  void shouldReturnOnlyWithNewStatus() throws Exception {
    ...
    when(leadService.findLeads(null, "NEW")).thenReturn(realService.findLeads(null, "NEW"));
  ...
  void shouldWorkWithBothFilters() throws Exception {
    ...
    when(leadService.findLeads("email3", "NEW")).thenReturn(expectedLeads);

    // WHEN,THEN
    mockMvc
        .perform(get("/leads").param("search", "email3").param("status", "NEW"))
        .andExpect(model().attribute("leads", expectedLeads))
        .andExpect(status().isOk());
  }
  ...
  void shouldReturnFormWithFieldErrorName() throws Exception {
    mockMvc
        .perform(post("/leads").param("name", "").param("email", "test@test.com"))
  ...
  void shouldReturnFormWithFieldErrorEmail() throws Exception {
    mockMvc
        .perform(post("/leads").param("email", "invalidemail"))
